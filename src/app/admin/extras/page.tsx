...
export default function AdminExtras() {
  ...
  const loadExtras = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/extras?page=${page}&limit=10&sort=${sort}&search=${encodeURIComponent(search)}`);
      if (!res.ok) throw new Error('Erro ao carregar extras');
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      alert('Por favor, preencha o nome do extra');
      return;
    }

    if (!isFree && (!price || Number(price) < 0)) {
      alert('Por favor, informe um preço válido para extras pagos');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error('Erro ao salvar extra');

      setOpen(false);
      loadExtras();
    } catch (error) {
      console.error('Erro ao salvar extra:', error);
      alert('Erro ao salvar extra');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Erro ao excluir extra');

      loadExtras();
    } catch (error) {
      console.error('Erro ao excluir extra:', error);
      alert('Erro ao excluir extra');
    }
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {loading && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-600">Carregando extras...</p>
          </div>
        )}
